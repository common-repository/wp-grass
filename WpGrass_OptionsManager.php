...
class WpGrass_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'wp-grass'));
        }
        ...
        <div class="wrap">
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'wp-grass'); ?></h2>
            <h3>There has been <em><?php echo $this->getDaysSinceLastPost(); ?></em> days since your last post</h3>
            ...
            <?php settings_fields($settingsGroup); ?>
                    ...
                    foreach ($optionMetaData as $aOptionKey => $aOptionMeta) {
                        ...
                        ?>
                            <tr valign="top">
                                ...
                                <td>
                                <?php $this->createFormControl($aOptionKey, $aOptionMeta, $this->getOption($aOptionKey,365)); ?>
                                </td>
                            </tr>
                        <?php
                    }
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'wp-grass') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function createFormControl($aOptionKey, $aOptionMeta, $savedOptionValue) {
        if (is_array($aOptionMeta) && count($aOptionMeta) >= 2) { // Drop-down list
            ...
            ?>
            <p><input type="number" name="<?php echo $aOptionKey ?>" id="<?php echo $aOptionKey ?>"
                      value="<?php echo esc_attr($savedOptionValue) ?>" size="50" min="8"/> days</p>
            <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'wp-grass');
            case 'false':
                return __('false', 'wp-grass');

            case 'Administrator':
                return __('Administrator', 'wp-grass');
            case 'Editor':
                return __('Editor', 'wp-grass');
            case 'Author':
                return __('Author', 'wp-grass');
            case 'Contributor':
                return __('Contributor', 'wp-grass');
            case 'Subscriber':
                return __('Subscriber', 'wp-grass');
            case 'Anyone':
                return __('Anyone', 'wp-grass');
        }
        return $optionValue;
    }
